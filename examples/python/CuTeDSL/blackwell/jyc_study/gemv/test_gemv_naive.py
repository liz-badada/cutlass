...
class Sm100DenseGemvKernelNaive:
    def __init__(self):
    ...
    def kernel(
        ...
        for k_tile in range(k_tile_cnt):
            ...
            tBgB = cute.make_tensor(tBgB_ptr, mma_tiler_mnk[2])

            a_val = tAgA.load()
            b_val_bf16 = tBgB.load()
            b_val = b_val_bf16.to(cutlass.Float32)

            for i in cutlass.range_constexpr(mma_tiler_mnk[2]):
                c_res += (a_val[i] * b_val[i])
...
def run_gemv_naive(
    ...
    """
    print("=" * 60)
    print("Launching Blackwell Float32 GEMV Naive Test")
    print("-" * 60)
    ...
    gemv = Sm100DenseGemvKernelNaive()
    # Initialize Stream
